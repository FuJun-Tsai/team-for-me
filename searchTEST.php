...
try{
    ...
    if('有會員登入'){
        $sql.= ', rc.MEMBER_NO as collectRES'; 
    }

    $sql.=' from `restaurant_management` R
    ...
    if('有會員登入'){
    $sql.= '
                LEFT OUTER JOIN restaurant_collection rc on ( R.RES_NO = rc.RES_NO)
                LEFT OUTER JOIN member_management mm on ( rc.MEMBER_NO = mm.MEMBER_NO)
            '; 
    }
